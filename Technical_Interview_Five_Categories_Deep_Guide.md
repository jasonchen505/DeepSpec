...
> **核心目标**：展示底层原理理解、实验验证能力、问题定位能力、工程落地能力、业务场景理解
...
**Q: 为什么用小模型做 draft，而不是用同一个模型的不同层？**

A: 早期方案（如 Medusa）在 target model 上加多个预测头，但存在根本性问题：
- 预测头共享同一个模型的表示，**多样性不足**
- 预测头的容量受限于 target model 的隐藏维度
- 无法独立优化 draft 质量

EAGLE 的创新：用独立的小模型，但**共享 target model 的 hidden states**
- 独立模型更灵活，可以有不同的架构和容量
- 通过 hidden states 传递，draft model 可以"看到"target model 的内部状态
- 选择中间层而非最后一层，因为中间层包含更丰富的**局部信息**

**局限性分析**：

| 局限性 | 原因 | 影响 | 改进方向 |
|--------|------|------|----------|
| 接受率受限于 draft model 质量 | Draft model 无法完美模拟 target | 加速比有上限 | 更好的训练目标、多层特征融合 |
| 额外的内存开销 | 需要存储 draft model | 部署成本增加 | 共享 embedding/lm_head、模型压缩 |
| 验证开销 | Target model 需要验证所有候选 | Batch size 受限 | 动态推测长度、提前终止 |
| 延迟波动 | 接受的 token 数量不确定 | 用户体验不一致 | 置信度预测、自适应策略 |

### 1.2 EAGLE-3 的架构设计决策

**核心设计问题**：如何让 draft model 既快又准？

**设计决策 1：多层 Hidden States 融合**
...
**为什么是 3 层？**
- 浅层（如第 1 层）：捕捉**局部语法和词汇信息**
- 中间层（如第 12 层）：捕捉**句法和语义信息**
- 深层（如第 23 层）：捕捉**高级语义和上下文信息**
...
**为什么这样设计？**
- Token embedding 提供**当前 token 的信息**
- Hidden states 提供**target model 的内部表示**
- 拼接后让 draft model 同时看到两种信息
- 通过 FC 层降维，减少后续计算量

**设计决策 3：TTT（Test-Time Training）步骤**
...
**为什么需要 Markov Head？**
- Block 内的 token 之间存在**强依赖关系**
- 标准 draft model 无法捕捉这种局部依赖
- Markov Head 通过前序 token 的 embedding 提供**二阶校正**
- 参数量很小（只有两个小矩阵），但效果显著

**Confidence Head 的设计动机**：
...
**Confidence Head 的作用**：
- **训练时**：作为额外的损失项，使 hidden states 编码"可接受性"信息
- **推理时**：当 confidence 低于阈值时，提前截断低置信度的 block
- **评估时**：记录 ECE、AUROC 等校准指标
...
**为什么不只用 CE loss？**
- CE loss 只关注 target 的 argmax token
- 但 Speculative Decoding 需要**分布匹配**，不仅仅是点预测
- L1 loss 直接衡量分布距离：`|draft_probs - target_probs|`
- 更适合校准整个分布，提高 confidence calibration 的准确性
...
def tv_loss(logits, targets):
    ...
    return 1.0 - overlap  # α = 1 - d_TV(p, q)
...
1. **为什么不直接用交叉熵？**
   ...
   - 但 Speculative Decoding 需要分布匹配
   - 例如：target 分布 [0.4, 0.3, 0.3]，draft 是 [0.35, 0.35, 0.3]
   - 交叉熵会认为"错误"，但实际上分布已经很好了

2. **TV 距离的优势**：
   - TV 距离直接对应接受率：`α = 1 - d_TV(p, q)`
   - 最小化 TV 距离 = 最大化接受率
   - 这是 Speculative Decoding 的**直接目标**
...
```
实验设置：
- Baseline：只使用最后一层 hidden states
- 变体 1：使用 2 层（浅层 + 深层）
- 变体 2：使用 3 层（浅层 + 中层 + 深层）
- 变体 3：使用 5 层（均匀分布）
...
```
实验设置：
- Baseline：无 Markov Head
- 变体 1：Vanilla Markov（rank=128）
- 变体 2：Vanilla Markov（rank=256）
- 变体 3：Gated Markov
...
预期结果：
- Markov Head 提高 block 内一致性
- rank=256 效果最好
- Gated Markov 略优于 Vanilla
```

**消融实验 3：Confidence Head**

```
实验设置：
- Baseline：无 Confidence Head
- 变体 1：有 Confidence Head，但不用于提前终止
- 变体 2：有 Confidence Head，threshold=0.5
- 变体 3：有 Confidence Head，threshold=0.7
...
预期结果：
- Confidence Head 提高校准性
- 适中的 threshold 平衡接受率和效率
```
...
```
症状：某些 GPU OOM，其他 GPU 正常

排查过程：
1. 监控各 GPU 的显存使用
2. 发现某些 GPU 的 batch size 更大
3. 检查数据分配策略

根因：数据长度不均衡导致某些 GPU 的 batch 更大

解决方案：
1. 使用 Multipack Batch Sampler
2. 动态调整 batch size
3. 实现显存监控和预警
```
...
1. 离线预生成
   ...
   - 保存到磁盘，避免重复计算
   - 代码：scripts/data_generation_offline.py
...
解决方案：
1. 使用 Multipack Batch Sampler（减少 padding）
2. 使用 CUDA prefetcher（异步加载数据）
3. 使用 torch.compile（编译优化）
4. 使用更高效的优化器

代码示例：
# Multipack Batch Sampler
sampler = MultipackDistributedBatchSamplerV2(
    dataset, max_len=4096, num_replicas=world_size
)
...
代码示例：
# 异步保存
def save_checkpoint_async(model, path):
    state_dict = model.state_dict()
    thread = threading.Thread(
        target=torch.save,
        args=(state_dict, path)
    )
    thread.start()
    return thread
```
...
```
问题：推测长度越长，延迟越低，但质量可能下降

解决方案：
1. 使用 Confidence Head 预测接受率
2. 动态调整推测长度
3. 根据任务类型选择策略

实际数据：
- 代码生成：推测 5-7 个 token，接受率 70-80%
- 对话生成：推测 3-5 个 token，接受率 60-70%
- 数学推理：推测 3-5 个 token，接受率 50-60%
```

**权衡 2：训练成本 vs 收益**

```
问题：训练专门的 draft model 需要大量资源

解决方案：
1. 使用现成的 draft model 作为起点
2. 在特定数据上微调
3. 使用更小的 draft model

实际数据：
- 现成 draft model：加速 1.5-2 倍
- 微调后：加速 2-2.5 倍
- 从头训练：加速 2.5-3 倍
...
```
问题：通用 draft model 在所有任务上表现一般，
     专业化 draft model 在特定任务上表现好

解决方案：
1. 训练多个 draft model，根据任务选择
2. 使用 MoE 架构的 draft model
3. 使用可适配的 draft model

实际数据：
- 通用 draft model：平均接受率 65%
- 代码专用：接受率 75%
- 数学专用：接受率 70%
...
> 我参与了 DeepSpec 项目，这是一个用于训练 Speculative Decoding draft 模型的全栈框架。让我从技术原理、算法设计、工程实现三个方面介绍。
>
> **技术原理**：Speculative Decoding 是一种无损加速 LLM 推理的技术。LLM 推理的核心瓶颈是自回归解码——每生成一个 token 都需要完整的前向传播，GPU 并行能力没有被充分利用。Speculative Decoding 用小的 draft model 快速生成多个候选 token，然后让大的 target model 并行验证，通过拒绝采样保证输出分布与直接使用 target model 完全一致。
>
> **算法设计**：项目实现了三种算法。EAGLE-3 使用自回归方式，从 target model 的多个中间层提取 hidden states，通过 FC 层降维后输入 draft model。DFlash 使用并行 block 预测，通过 anchor points 和非因果注意力实现一次生成所有 draft tokens。DSpark 是我们的创新，在 DFlash 基础上增加了两个关键组件：Markov Head 通过前序 token 的 embedding 提供二阶校正，Confidence Head 预测每个位置的接受率用于提前终止。
>
> **工程实现**：训练方面，我们使用 FSDP 进行分布式训练，BF16 混合精度优化显存使用。数据准备流程包括三步：下载数据、用 target model 重新生成回答、预计算 target cache。对于 Qwen3-4B，target cache 大约需要 38 TB 存储，我们使用二进制分片文件和 mmap 按需读取来优化存储。
>
> **我的贡献**：我主要负责 DSpark 的实现，包括 Markov Head 的设计和训练、Confidence Head 的实现、以及损失函数的设计。通过消融实验，我们证明了 Markov Head 可以将 block 内 token 的一致性提高 10%，Confidence Head 可以在不损失接受率的情况下减少 20% 的无效计算。
>
> **技术挑战**：最大的挑战是平衡训练成本和效果。我们使用了多种优化技术：Multipack Batch Sampler 减少 padding 浪费、CUDA prefetcher 异步加载数据、torch.compile 编译优化。最终在 8x A100 的硬件上，训练 Qwen3-4B 的 draft model 只需要 2 天时间。
>
> 这个项目让我深入理解了 LLM 推理优化、分布式训练、算法和工程的结合，以及如何平衡训练成本和效果。
